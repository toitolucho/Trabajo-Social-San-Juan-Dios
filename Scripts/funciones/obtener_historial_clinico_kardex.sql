CREATE OR REPLACE FUNCTION obtener_historial_clinico_kardex(
	numero_hc_aux	INT,
	fecha_inicio		DATE,
	fecha_fin		DATE
...
----'N'->NINGUNO, 'A'->ANALFABETO, 'P'->PRIMARIO, 'S'->SECUNDARIO, 'U'->UNIVERSITARIO, 'T'->TECNICO SUPERIOR, 'R'->SUPERIOR    --GRADO INSTRUCCIÓN
			...
			hc.numero_hc, hc.grupo_sanguinio, hc.fecha_registro
		FROM personas_kardex pk
		INNER JOIN personas p
		ON pk.codigo_persona = p.codigo_persona
		INNER JOIN historia_clinico hc
		on hc.idpersona = p.codigo_persona
		...
		WHERE hc.numero_hc = numero_hc_aux
	LOOP
...
-- 
SELECT * FROM obtener_historial_clinico_kardex(1, null, null) 
AS 
(
