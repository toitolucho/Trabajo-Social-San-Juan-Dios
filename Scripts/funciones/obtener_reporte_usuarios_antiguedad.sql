CREATE OR REPLACE FUNCTION obtener_reporte_usuarios_antiguedad(
...
BEGIN
	...
	sql = 'select u.codigo_usuario, u.fecha_registro, u.nombre_usuario, u.contrasenia,
			...
			case when date_part(''year'', age(u.fecha_registro)) =  0 then ''0-1 años''
			when date_part(''year'', age(u.fecha_registro)) between 1 and 5 then ''1-5 años''
			when date_part(''year'', age(u.fecha_registro)) between 6 and 10 then ''6-10 años''
			when date_part(''year'', age(u.fecha_registro)) between 11 and 15 then ''11-15 años''
			else ''15 - mas'' end
		from usuarios u
...
SELECT * FROM obtener_reporte_usuarios_antiguedad(NULL, NULL, NULL) 
AS 
(	
...
-- 
-- select u.codigo_usuario, u.fecha_registro, u.nombre_usuario, u.contrasenia,
-- 	CASE u.codigo_tipo WHEN 'S' THEN 'SECREATRIA' WHEN 'M' THEN 'MEDICO' WHEN 'V' THEN 'TRABAJADORA SOCIAL' WHEN 'A' THEN 'ADMINISTRADOR(A)'
-- 	WHEN 'D' THEN 'DIRECTORA DEL HOGAR' WHEN 'P' THEN 'PSICOLOGA' END,
-- 	p.di_persona, p.nombres, p.paterno, p.materno, p.nombres || ' ' || p.paterno || ' ' || p.materno, p.paterno || ' ' || p.materno,
-- 	e.nombre_empleo, CASE p.grado_instruccion WHEN 'N' THEN 'NINGUNO' WHEN 'A' THEN 'ANALFABETO' WHEN 'P' THEN 'PRIMARIO' WHEN 'S' THEN 'SECUNDARIO'
...
-- on p.codigo_empleo = e.codigo_empleo
-- 
-- select age(u.fecha_registro), date_part('year', age(u.fecha_registro))
-- from usuarios u